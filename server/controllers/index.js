...
const setName2 = (req, res) => {
  ...
  // normally you would also perform validation
  // to know if the data they sent you was real
  if (!req.body.name || !req.body.breed || !req.body.age || !req.body.date) {
    // if not respond with a 400 error
    // (either through json or a web page depending on the client dev)
    return res.status(400).json({ error: 'name, breed, age, and date are all required' });
  }
  ...
  savePromise.then(() => {
    // set the lastAdded2 dog to our newest cat object.
    // This way we can update it dynamically
    lastAdded2 = newDog;
    // return success
    res.json({
      name: lastAdded2.name, breed: lastAdded2.breed, age: lastAdded2.age, date: lastAdded2.date,
    });
  });
...
const searchName2 = (req, res) => {
  ...
  return Dog.findByName(req.query.name, (err, doc) => {
    ...
    // if a match, send the match back
    return res.json({
      name: doc.name, breed: doc.breed, age: doc.age, date: doc.date,
    });
  });
};
...
const updateLast2 = (req, res) => {
  ...
  // then just call the Model object's save function
  // create a new save promise for the database
  const savePromise = lastAdded2.save();
